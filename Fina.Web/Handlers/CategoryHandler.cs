...
public class CategoryHandler(IHttpClientFactory httpClientFactory) : ICategoryHandler
{
    ...
    public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
         => await _client.GetFromJsonAsync<PagedResponse<List<Category>?>>($"v1/categories/{request}")
            ?? new PagedResponse<List<Category>?>(null, 400, "Erro ao buscar Id de categoria");

    public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
        => await _client.GetFromJsonAsync<Response<Category?>>($"v1/categories/{request.Id}")
            ?? new Response<Category?>(null, 400, "Erro ao buscar Id de categoria");
